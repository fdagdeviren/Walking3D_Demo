...
#region Fields
        ...
        private NavMeshPath m_path;                         // nav mesh path info class
...
#region Unity Methods
            ...
            if (m_Aiming)
            {
                ...
                if (ChaseTransform)
                {
                    if (isOnDestination())
                    {
                        ...
                        NavMeshHit nhit;
                        if (NavMesh.SamplePosition(dest, out nhit, 120.0f, NavMesh.AllAreas))
                        {
        ...
        private bool isOnDestination()
        {
            ...
            if (!NavMesh.CalculatePath(transform.position, m_CurrentDest, NavMesh.AllAreas, m_path))
            {
#if DEBUG_INFO
